...
if nargin < 2 || isempty(regularization)
    regularization = eps;
elseif strcmp(method, 'TAUB') && regularization > eps
...
% Check data size
if any(size(data) < 2)
    error('All data dimensions have to be >= 2');
end
...
Me = nan; oe = nan; dist = inf;
success = false;
A = nan; regParam = nan; %#ok
...
if p.nDim ~= size(data,2) ...
    ...
    p.regMatrix((p.nDim + (0:p.nCrossTerms)) * (tmp+1)+1) = ...
        p.nDim;
...
for ii = 1:numel(I)
    d = GetDiscriminants(evec(:,I(ii)), p);
    if any(d < 0)
        I(ii) = -1;
    end
end
